...
export function registerResumeRoutes(app: Express) {
  ...
  app.post("/api/resume/upload-analyze", (req: any, res: any, next: any) => {
    ...
    try {
      ...
      const llmOpts: Parameters<typeof createOpenAI>[0] = {
        apiKey: ENV.llmApiKey || "no-key",
      };
      if (ENV.llmBaseUrl) {
        llmOpts.baseURL = ENV.llmBaseUrl.endsWith("/v1") ? ENV.llmBaseUrl : `${ENV.llmBaseUrl}/v1`;
      }
      const openai = createOpenAI(llmOpts);
